        ...
        for k in range(len(obs)):
            ...
            if(fl == True):
                if(b == 0 ):
                    ...
                    actions.append(self.actions[-(next_node[1] - positions_xy[k][1]),(-(next_node[0] - positions_xy[k][0]))])
                elif(b== 2):
                    actions.append(self.actions[(next_node[1] - positions_xy[k][1]),((next_node[0] - positions_xy[k][0]))])
                else:
                    actions.append(self.actions[(next_node[0] - positions_xy[k][0], next_node[1] - positions_xy[k][1])])
...
def main():
    ...
    grid_config = GridConfig(num_agents=22,  # количество агентов на карте
                             size=59,  # размеры карты
